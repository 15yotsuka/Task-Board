...
## 現在の状態
- 開発フェーズ: App Store提出準備完了（審査提出待ち）
- 最終更新: 2026-03-22
- 動作ステータス: ✅ TypeScriptエラー0件
...
### App Store提出（残り手動作業）
1. [Yu手動] App Store Connect → プライバシーポリシーURL を入力
2. [Yu手動] App Store Connect → ビルドを選択（altoolでアップロード済み、処理完了後に表示）
3. [Yu手動] App Store Connect → 連絡先情報を記入（名前・電話・メール）
4. [Yu手動] App Store Connect → コンテンツの配信権を設定
5. [Yu手動] App Store Connect → アプリのプライバシー（データ収集なし）を設定
6. [Yu手動] 「審査へ提出」ボタンをクリック
7. [Yu手動] AdMob お支払いプロファイルを完成させる（警告バナーが出ている）
