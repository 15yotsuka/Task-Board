...
export function TaskDetailModal({ todo, visible, onClose }: Props) {
  ...
  const [isDateOnly, setIsDateOnly] = useState(true);
  ...
  useEffect(() => {
    if (todo) {
      ...
      setIsDateOnly(todo.isDateOnly ?? true);
      if (todo.dueDate) {
  ...
  const handleSave = () => {
    ...
    updateTodo(todo.id, {
      ...
      groupId,
      dueDate: dueDateIso,
      isDateOnly: dueDate ? isDateOnly : false,
    ...
    void (async () => {
      ...
      if ((notificationsEnabled || validMinutes !== null) && dueDate) {
  ...
  const handleDelete = () => {
    Alert.alert(
      t("task.deleteAlert.title"),
      t("task.deleteAlert.message", { title: todo.title }),
      [
      ...
      <View style={styles.priorityRow}>
        {PRIORITY_OPTIONS.map((opt) => {
          ...
          return (
            ...
            >
              <Text style={[styles.priorityChipText, { color: pc.text }]}>
                {opt.label}
              </Text>
            </Pressable>
          );
          ...
          <>
            ...
            >
              <Text
                style={[
                  styles.dateText,
                  { color: dueDate ? theme.text : theme.secondaryText },
                ]}
              >
                {dueDate
                  ? isDateOnly
            ...
            {dueDate && !showDatePicker && (
              <>
                <View style={styles.timeToggleRow}>
                  ...
                  <Switch
                    value={!isDateOnly}
                    onValueChange={(v) => setIsDateOnly(!v)}
            ...
            {showDatePicker && Platform.OS === "ios" && (
              ...
              >
                ...
                >
                  ...
                  >
                    <Text
                      style={[styles.pickerBtn, { color: theme.secondaryText }]}
                    >
                      {t("common.cancel")}
                    </Text>
                  </Pressable>
                  <Pressable
                    onPress={() => {
                      ...
                      if (!isDateOnly) {
                        d.setHours(
                          pendingTime.getHours(),
                          pendingTime.getMinutes(),
                          0,
                          0,
                        );
                      } else {
            ...
            {groups.length > 0 && (
              <>
                <Text
                  style={[styles.fieldLabel, { color: theme.secondaryText }]}
                >
                  {t("common.group")}
                </Text>
                <View style={styles.categoryRow}>
                  <Pressable
                    ...
                    style={({ pressed }) => [
                      ...
                      {
                        backgroundColor:
                          groupId === null ? theme.primaryBg : "transparent",
                        borderColor: theme.border,
                        opacity: pressed ? 0.7 : 1,
                      },
                    ]}
                  >
                    <Text
                      style={[styles.categoryChipText, { color: theme.text }]}
                    >
                      {t("common.none")}
                    </Text>
                  </Pressable>
                        ...
                        styles.categoryChip,
                        {
                          backgroundColor:
                            groupId === g.id
                              ...
                              : "transparent",
                          borderColor:
                            groupId === g.id ? g.color : theme.border,
                          opacity: pressed ? 0.7 : 1,
                        },
                      ]}
                    >
                      <View
                        style={[styles.catDot, { backgroundColor: g.color }]}
                      />
                      <Text
                        style={[styles.categoryChipText, { color: theme.text }]}
                      >
            ...
            <View style={styles.categoryRow}>
              <Pressable
                ...
                style={({ pressed }) => [
                  ...
                  {
                    backgroundColor:
                      categoryId === null ? theme.primaryBg : "transparent",
                    borderColor: theme.border,
                    opacity: pressed ? 0.7 : 1,
                  },
                ]}
                    ...
                    styles.categoryChip,
                    {
                      backgroundColor:
                        categoryId === cat.id
                          ...
                          : "transparent",
                      borderColor:
                        categoryId === cat.id ? cat.color : theme.border,
                      opacity: pressed ? 0.7 : 1,
                    },
                  ]}
                >
        ...
        {activeTab === 1 && (
          <>
            <>
              ...
              <View style={styles.notifPresetRow}>
                <Pressable
                  ...
                  style={({ pressed }) => [
                    ...
                    {
                      backgroundColor:
                        notifMinutes === null ? theme.primaryBg : theme.pageBg,
                      borderColor:
                        notifMinutes === null ? theme.primary : theme.border,
                      opacity: pressed ? 0.7 : 1,
                    },
                  ]}
                >
                  <Text
                    style={[
                      ...
                      {
                        color:
                          notifMinutes === null ? theme.primary : theme.text,
                      },
                    ]}
                  >
                    {"通知なし"}
              ...
              {notifMinutes === null && notificationsEnabled && (
                <Text
                  style={[styles.notifHint, { color: theme.secondaryText }]}
                >
                  {"設定のデフォルト（" +
        ...
        {activeTab === 2 && (
          <>
            ...
            <Text style={[styles.detailText, { color: theme.text }]}>
              {formatFullDate(todo.createdAt, locale)}
            </Text>
            ...
            <Text style={[styles.detailText, { color: theme.text }]}>
              {formatFullDate(todo.updatedAt, locale)}
            </Text>
          </>
        )}
      </View>
