...
const P8_PATH =
  "/Users/yuotsuka/.appstoreconnect/private_keys/AuthKey_C9WM6RT2H7.p8";

// ── 入力するメタデータ ────────────────────────────────────
const LOCALE = "ja";

const PROMO_TEXT =
  "カレンダーでやることを一望。期限・優先度でタスクを自動整理。今日・今週・今月別にセクション表示。グループ・カテゴリで色分け管理。シンプルで使いやすい予定管理アプリ。";

const DESCRIPTION = `かんたんタスクカレンダーは、カレンダーとタスク管理を一体化したシンプルな予定管理アプリです。

【カレンダー表示】
月表示・週表示の2つのカレンダービューで、タスクの締切日をビジュアルで確認できます。予定とやることを一画面で把握できます。

【自動セクション分け】
タスクを「期限超過」「今日」「今週」「今月」「それ以降」のセクションに自動で振り分けます。やることが一目でわかるので、毎日の管理がスムーズになります。
...
【通知リマインダー】
締切の何分前に通知するかを設定できます。タスクごとに個別設定も可能です。
...
const KEYWORDS =
  "タスク管理,ToDo,スケジュール,リマインダー,優先度,締切,予定表,やること管理,タスクリスト,仕事効率化";
const SUPPORT_URL = "https://15yotsuka.github.io/Task-Board/";
const COPYRIGHT = "© 2026 Yu Otsuka";
const APP_NAME = "かんたんタスクカレンダー";
const SUBTITLE = "予定とタスクをシンプルに管理";
...
async function main() {
  ...
  const apps = await request(
    "GET",
    "/v1/apps?fields[apps]=bundleId,name&limit=50",
    null,
    token,
  );
  const app =
    apps.data.find((a) => a.attributes.bundleId === "com.yuotsuka.taskboard") ||
    apps.data.find(
      (a) =>
        a.attributes.name?.includes("タスク") ||
        a.attributes.name?.includes("Task"),
    );
  ...
  if (!app) {
    ...
    apps.data.forEach((a) =>
      console.log(
        `  - ${a.attributes.name} (${a.attributes.bundleId}) id=${a.id}`,
      ),
    );
    throw new Error(
      "タスクボードが見つかりません。上のリストからApp IDを確認してください。",
    );
  }
  ...
  if (!versions.data.length)
    throw new Error("App Store バージョンが見つかりません");
  ...
  if (appInfos.data.length > 0) {
    ...
    if (aiLoc) {
      await request(
        "PATCH",
        `/v1/appInfoLocalizations/${aiLoc.id}`,
        {
          data: {
